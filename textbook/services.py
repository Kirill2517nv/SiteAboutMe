...
def visible_articles(user=None):
    ...
    Учителю отдаём и спрятанное: вычитывать перед публикацией он должен на том
    же сайте, а не в админке, – и `is_published` самой статьи снимается для него
    по той же причине. Иначе снятая с публикации статья (разборы задач
    спецкурса лежат так, пока не вычитаны) отдавала бы 404 и автору тоже,
    а вычитывать вёрстку, формулы и картинки в поле админки нечем.

    Отсюда правило для новых вызовов: функция отвечает на вопрос «можно ли
    показать этому человеку эту статью», а не «что показать в списке». Черновик
    попадает в выдачу только там, где адрес назвали явно – открытие статьи по
    slug и соседи вокруг уже открытой. Список, который собирается сам (главная
    учебника, карточки задач спецкурса), фильтрует `is_published` у себя, иначе
    невычитанное всплывёт у учителя в оглавлении и он выпустит его, не заметив.
    """
