...
- **Theory** – `textbook.EgeTask` (1–27) + `Article(track='ege')`. Article template and writing rules: `docs/ege-theory-brief.md`. **Несколько разборов у одного номера** (задание 26: основной + `seed_ege_theory_26_serii` / `_matreshka` / `_reyting`): заглушку номера `seed_ege_tasks` находит как *первую* статью задания по `(order, title)`, поэтому дополнительный разбор создаёт свою статью сам (`update_or_create` по slug) и обязан иметь `ORDER` больше номера – иначе он встанет перед основным, будет принят за заглушку и переименован в «Задание N. …». Код из статьи с файлом данных прогоняется общим `_seed_check.run_solution` (временный каталог, файл под тем же именем, что скачает ученик); подчёркивание в имени модуля не даёт Django счесть его командой
...
- **Реестр виджетов грузится по условию** – `static/js/textbook-widgets.js` это 33 виджета и 161 КБ gzip, а виджет стоит в четверти статей (65 из 226): на остальных скрипт скачивался, не находил ни одного `[data-widget]` и выходил. `article_detail.html` держит тег и наблюдатель MathJax (который живёт ровно ради виджетов) под `{% if has_widgets %}`, а флаг считает `article_detail_view` – `any(b.block_type == 'widget' for b in blocks)` по **тому же** списку, который шаблон и рисует. Второй способ узнать «есть ли тут виджет» заводить нельзя: `visible_blocks` отдаёт суперпользователю ещё и черновики, и любая независимая проверка (запрос в базу, `article.blocks.filter(...)`) разошлась бы с разметкой – у автора статьи виджет на странице есть, а скрипта нет. `WidgetScriptLoadingTests` сторожит обе стороны
- **Иллюстрация статьи показывается целиком, общего бокса у картинок нет.** `.article-body figure img` держал `aspect-ratio: 4/3` с `object-fit: cover`, и каждая картинка не этой пропорции теряла ровно то, ради чего нарисована (таблица ASCII в уроке 7.2, лента цепочек в 8.3, окно turtle в разборе задания 6) – а исключения заводились по имени файла, и список рос с каждой новой картинкой. Осталась одна страховка `max-height: 80vh` против очень высокого скриншота. Раз пропорцию больше не диктует CSS, место под картинку резервируют атрибуты `width`/`height` у `img`, иначе текст прыгает при загрузке; **их и размеры лайтбокса считает `article_detail_view`** из самого файла (`block.pswp_size`): на зашитых в шаблон 1600×1200 PhotoSwipe растягивал любой другой кадр. Чтение размеров обёрнуто в `try` – у блока бывает запись о картинке без файла на диске (сид прогнали раньше, чем приехал media), и одна битая иллюстрация не должна ронять страницу
- **Отчёт учителя по блоку `/textbook/section/<slug>/stats/`** (`section_stats_view`, суперпользователи) – таблица учеников по классам, **один класс за раз** (`?group=<id>`, вкладки над таблицей, по умолчанию первый класс; `none` – «Без группы», `all` – все сразу), ровно как `/ege/class/`: пять таблиц подряд перестают отвечать на свой вопрос. Мусор в `?group=` трактуется как «все», а не как 404 – ссылка на класс переживает удаление класса. Строки сортируются в Python по той же строке, что стоит в ячейке (`surname_first`: «Фамилия Имя», иначе логин; ё = е), а не `order_by('last_name')` – тот ставил учеников без фамилии в начало, хотя в таблице у них логин. **Фамилия ведёт не в профиль, а в разбор ответов** (`section_stats_errors_view`, `/stats/<user_id>/errors/`): из таблицы идут смотреть работу ученика, а не карточку человека; на профиль осталась кнопка справа в шапке самого разбора. У разбора два независимых среза, и они складываются: `?errors=1` – только ошибки (на неё ведёт счётчик ошибок в таблице, поэтому число задач на странице обязано совпасть с числом на кнопке), `?kind=practicum|selfcheck` – практикум против самопроверок (домашнюю задачу решают, вопрос по ходу чтения щёлкают, в одном списке их сравнивать не с чем). Адреса вкладок собираются во вьюхе, а не в шаблоне: у страницы два параметра, и каждая вкладка обязана сохранить второй. **«Решена ли задача» считается по всем ответам, а не по показанным** – в режиме ошибок верной попытки в выборке нет, а «решил позже» показать надо; отсюда же бейдж «Решена» вместо «Решил позже», когда ошибок у задачи не было. `SectionStatsTeacherTests` сторожит выбор класса, ссылку с фамилии и оба среза

- **spetskurs** – годовой курс численного моделирования на C++, смонтирован на `/spetskurs/`. Одна модель `CourseTask` (бывшая `Simulation`) связывает физику, исходник `main.cpp`, WASM-симуляцию и задания. `TheoryPage`/`TheoryBlock` удалены: это была урезанная копия `textbook.Article`/`ArticleBlock`, из которой учебник когда-то и вырос, – **теория спецкурса живёт на моделях учебника** (`Article(track='spetskurs')`), и ей достались типографика, MathJax, прогресс чтения и показ с проектора. FK `Article.course_task` делит статьи надвое: заполнен – разбор задачи, пусто – «Основы C++» (`/spetskurs/basics/`), язык в объёме, нужном чтобы прочитать свой `main.cpp`.
  - **Разборы выпускаются по одной статье, когда вычитаны.** Пока `is_published=False`, лендинг, карточка в `/spetskurs/tasks/` и страница задачи говорят «Разбор готовится», а симуляция работает: задача на курсе есть, не готов только текст. Счётчик материалов на карточке считает **опубликованные** статьи тем же `Prefetch`, что и список под ней (`_published_articles`): обратная связь `task.articles.count` фильтра публикации не знает и обещала бы «7 материалов» у задачи, разбора которой на сайте нет
  - **`seed_spetskurs_<задача>` создаёт черновики**: `is_published` стоит в `create_defaults`, а не в `defaults`, поэтому повторный прогон правит текст и **не** снимает с публикации уже выпущенное. Гонять после каждой правки безопасно. `seed_spetskurs_basics` публикует сразу – «Основы» вычитаны. Проверка написанного: `python manage.py check_articles --track spetskurs` (формулы, ссылки, порядок блоков – по **базе**, а не по тестовой БД). Выпуск – `python manage.py publish_spetskurs <задача> [слаги статей]`: без слагов выходит весь разбор, со слагами – по одной вычитанной статье; опечатка в слаге падает CommandError, а не тихо публикует ноль строк
  - **Вычитывают статью на сайте, а не в поле админки**: `visible_articles` отдаёт суперпользователю и неопубликованное, и соседи в сайдбаре считаются той же функцией. Правило для новых вызовов: функция отвечает «можно ли показать эту статью этому человеку», а не «что показать в списке» – черновик виден только там, где адрес назвали явно; список, который собирается сам, фильтрует `is_published` у себя
  - **Кадр симуляции** – `_simulation_frame.html`, iframe с WASM-сборкой на том же домене. Колесо над кадром масштабирует график и не должно двигать страницу: библиотека курса это уже пробует (`emscripten_set_wheel_callback` + `return EM_TRUE`), но вешает обработчик на `window` внутри iframe, а Chrome делает wheel-слушатели на `window`/`document`/`body` пассивными – там `preventDefault()` запрещён и молча игнорируется. Поэтому родитель ставит в документ кадра свой слушатель с явным `{ passive: false }`; дельту по-прежнему читает симуляция, отменяется только прокрутка. Выглядит дублем библиотечного EM_TRUE и первым просится под нож – сторожит `SimulationWheelTest`
  - **Архив исходников библиотеки** – `static/spetskurs/GuiLibrary-src.zip`, ссылка в статье «Программа, сборка и запуск» рядом с `git clone`: в школе GitHub закрыт. Лежит вместе с `external/` (подмодули внутри, Git не нужен), в git не коммитится – `static/spetskurs/*.zip` в `.gitignore`, выкладывается тем же rsync, что и `.wasm`. Слепок сам не обновляется: пересобирать вручную
  - Выкладка `.wasm`, архива и порядок выпуска разборов: `docs/spetskurs-deploy.md`
